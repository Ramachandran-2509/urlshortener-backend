...
app.post("/users/forgot", async (request, response) => {
  ...
  if (user.length > 0) {
    ...
    let mailOptions = {
      ...
      html:
      '<a href = "https://urlshortener-2425.netlify.app/retrieveAccount/' + email + '/' + token + '"> Reset Password Link</a>'
    };
...
app.post("/users/SignUp", async (request, response) => {
  ...
  const token = jwt.sign({ email: email }, process.env.MY_SECRET_KEY);
  const url = `https://urlshortener-2425.netlify.app//activateAccount/${email}/${token}`;
  const client = await createConnection();
  const salt = await bcrypt.genSalt(10);
